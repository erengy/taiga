...
BOOL SettingsPage::OnInitDialog() {
  switch (index) {
    ...
    case PAGE_FOLDERS_ANIME: {
      win32::ListView List = GetDlgItem(IDC_LIST_FOLDERS_ANIME);
      ...
      for (int i = mal::MYSTATUS_WATCHING; i <= mal::MYSTATUS_PLANTOWATCH; i++) {
        if (i != mal::MYSTATUS_UNKNOWN) {
          List.InsertGroup(i, mal::TranslateMyStatus(i, false).c_str(), true, i != mal::MYSTATUS_WATCHING);
        }
      }
    ...
    case PAGE_MEDIA: {
      ...
      List.SetImageList(UI.ImgList16.GetHandle());
      List.SetTheme();
      win32::Window Header; HDITEM hdi = {0};
