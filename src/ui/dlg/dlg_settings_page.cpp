...
BOOL SettingsPage::OnInitDialog() {
  switch (index) {
    ...
    case kSettingsPageLibraryManagement: {
      CheckDlgButton(IDC_DELETE_WATCHING, taiga::settings.GetManagementDeleteAfterWatching());
      CheckDlgButton(IDC_DELETE_COMPLETED, taiga::settings.GetManagementDeleteAfterCompletion());
      CheckDlgButton(IDC_DELETE_PROMPT, taiga::settings.GetManagementPromptDeletion());
      CheckDlgButton(IDC_DELETE_PERMANENT, taiga::settings.GetManagementDeletePermanently());
      SendDlgItemMessage(IDC_SPIN_KEEPNUM, UDM_SETRANGE32, 0, 1000); // Hardcoded upper limit maybe bad/too small?
      SendDlgItemMessage(IDC_SPIN_KEEPNUM, UDM_SETPOS32, 0, taiga::settings.GetManagementKeepNum());
      break;
    }
